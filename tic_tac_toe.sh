#!/bin/bash
# Game of tic-tac-toe by someone
  ...
  if [ "$game_finished" = false ] ; then
    ...
    if [ "$stall" = false ] ; then
      if [ "$player_one" = true ] ; then
        prompt="${bold}Your move, "$player_1_str"?${normal}"
        sign="X"
        player_one=false
      else
        prompt="${bold}Your move, "$player_2_str"?${normal}"
        sign="O"
        player_one=true
      fi
    else
...
function test_position_str {
  ...
  if [[ $rows =~ [X]{3,} || $cols =~ [X]{3,} || $diagonals =~ [X]{3,} ]]; then
    end_game
    echo -e $player_1_str" ${bold}WINS!${normal} \n"
    return
  fi
  if [[ $rows =~ [O]{3,} || $cols =~ [O]{3,} || $diagonals =~ [O]{3,} ]]; then
    end_game
    echo -e $player_2_str" ${bold}WINS!${normal} \n"
    return
  fi
  if [[ ! $positions_str =~ [-] ]]; then  # all positions taken, not one has won
    end_game
    echo -e "${bold}End with a${normal} "$pink"${bold}DRAW${normal}"$reset"\n"
  fi
}
